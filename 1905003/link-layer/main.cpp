...
void showElements(vector<int> vect, HANDLE hConsole);
...
string toggleBits(const string& inputString, double probability, vector<int>& errorBits, HANDLE hConsole);
int binaryStringToAscii(const string& binaryString);
void printRecoveredMsg(const vector<vector<int>>& inputVector);
...
int main() {
    ...
    for(int i = 0; i < numRows; i++){
        ...
        showElements(hammingCode.back(), hConsole);
        cout << endl;
    }
    ...
    string rxFrame = toggleBits(txFrame, p, errorBits, hConsole);
    cout << endl;
...
// Function to traverse and print a 2D binary vector in row-major order as a collection of ascii characters
void printRecoveredMsg(const std::vector<std::vector<int>>& matrix) {
