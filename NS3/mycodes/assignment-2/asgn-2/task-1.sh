...
for i in "${arr[@]}";
do
    ./ns3 run "scratch/asgn-2/task-1.cc --tcp2=${tcp} --nLeaf=$2 --bttlnkRate=${i} --output_folder=${path} --exp=1" >> "${path}/${method}.txt" 2>&1
...
for i in {2..6}
do
    ./ns3 run "scratch/asgn-2/task-1.cc --tcp2=${tcp} --nLeaf=$2 --plossRate=${i} --output_folder=${path} --exp=2">> "${path}/${method}.txt" 2>&1
    # output file name | file 1 | file1_1stcol | file1_2ndcol | title 1 | file2 | file2_2ndcol | title 2
